        ...
        switch (filter)
        {
          case 0:
            ...
            else
            {
              ...
              check_menu_item_set_active ((gpointer)this_proj -> modelgl -> ogl_lab[is_clone][sid], ! status);
                                  ...
                                  gpointer data)
{
  ...
  if (i > 0)
  {
    i --;
    // For atom i
    adjust_this_tree_leave (asearch, this_proj, dat -> b, i, -1, iter);
  }
  else
  {
    // For spec i
    if (asearch -> todo_size >= 10000 && ! asearch -> passivating)
    {
      ...
      if (gtk_tree_model_iter_children (GTK_TREE_MODEL(asearch -> atom_model), & child, & iter))
      {
        ...
        while (dothis)
        {
          ...
          k --;
          adjust_this_tree_leave (asearch, this_proj, dat -> b, k, ! j, child);
                              ...
                              gpointer data)
{
  ...
  if (gtk_tree_model_get_iter (model, iter, path))
  {
    ...
    {
      i --;
      adjust_this_tree_leave (asearch, this_proj, dat -> b, i, asearch -> int_b, * iter);
    }
